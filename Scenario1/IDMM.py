...
#驾驶员舒适的减速度
Comfortable_Dec=1.67

# 最短的车头间距
Minimum_Spa=2#最短车头间距
...
while True:
    ...
    Des_headtime=Minimum_headtime*(Aadpt_factor+los*(1-Aadpt_factor))

    cur_des_spa = Minimum_Spa + cur_spe * Des_headtime + 0 * math.sqrt(cur_spe / Desire_Spe) + \
                  (cur_spe * (cur_spe - forehead_spe)) / (2 * math.sqrt(Max_Acc * Comfortable_Dec))

    cur_acc = Max_Acc * (1 - math.pow((cur_spe / Desire_Spe), Acc_para) -
                             math.pow(cur_des_spa / (cur_head_space-Eff_veh_len), 2))
...
data=pd.DataFrame({"仿真时间":time_list,"IDM加速度":acceleration_list,"IDM速度":speed_list,
                   "IDM车头间距":head_space_list,"IDM绝对距离":absolute_distance_list})

data.to_excel("C:\\Users\\cc_01\\Desktop\\基准分析\\场景1，启动，加速，自由流，停车\\输出文件\\IDMM.xlsx")
