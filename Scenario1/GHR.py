...
cur_acc,cur_spe,cur_head_space,absolute_distance=0,25,forehead_dis,0
...
pre_spe_list=[cur_spe]*100
pre_rel_dis=[cur_head_space - Eff_veh_len]*100

while True:
    # 更新每一次的速度，加速度，车头间距，行驶距离

    cur_acc = (para_alpha *cur_spe* (pre_spe_list[0] ** para_spe)) / ((pre_rel_dis[0]) ** para_dis)
    cur_acc=-cur_acc
    cur_spe=cur_spe+cur_acc*0.01

    if round(cur_spe,5)==0:
        break
    ...
    if ((absolute_distance - last_absolute_distance) <= 0.5) and absolute_distance> 10000:
        print("后车停车")
        print("后车在前车+" + str(round(cur_head_space - Eff_veh_len,4)) + "m处停止，" + "数值仿真结束，共用时长为" + str(simulation_time))
        break
    # ————————————————判断数值计算是否结束————————————————————#
...
data.to_excel("C:\\Users\\cc_01\\Desktop\\基准分析\\数值仿真\\数值仿真\\场景1，启动，加速，自由流，停车\\输出文件\\10公里\\GHR.xlsx")
