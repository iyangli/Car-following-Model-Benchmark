...
forehead_total_time=ini_second_time+abs(forehead_sec_car_de_speed-forehead_sec_car_speed)/abs(forehead_acc_increase)
...
path="C:\\Users\\cc_01\\Desktop\\基准分析\\数值仿真\\数值仿真\\场景2，跟驰，被抢道，跟驰，两辆车\\输出文件\\统计分析,version-8\\FVD-cut_in=20.xlsx"
...
def get_forehead_spe_acc(time,ini_first_time,ini_second_time,acc_increase,
    ...
    if time >=ini_second_time and time <=(ini_second_time+abs((sec_car_de_speed-sec_car_speed)/acc_increase)):
...
while time <=forehead_total_time:
    ...
    print("最优速度为"+str(cur_bef_des_spe))
