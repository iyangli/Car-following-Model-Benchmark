...
# 模型默认的参数值
Length_of_Car=5 #车长
Maximum_Acc=0.73#最大加速度
Comfortable_Dec=1.67#舒适减速度
Desire_Spe=31#期望速度
Desire_Spa_Tim=1.6#期望车头时距
...
path="C:\\Users\\cc_01\\Desktop\\基准分析\\数值仿真\\数值仿真\\场景2，跟驰，被抢道，跟驰，两辆车\\输出文件\\统计分析,version-8\\IDM-cut_in=10.xlsx"
...
while time <=forehead_total_time:
    ...
    # 后车参数的更新
    cur_des_spa = Minimum_Spa + cur_spe * Desire_Spa_Tim + 0 * math.sqrt(cur_spe / Desire_Spe) + \
                  (cur_spe * (cur_spe - forehead_spe)) / (
                          2 * math.sqrt(Maximum_Acc * Comfortable_Dec))

    cur_acc = Maximum_Acc * (1 - math.pow((cur_spe / Desire_Spe), Para_Beta) -
